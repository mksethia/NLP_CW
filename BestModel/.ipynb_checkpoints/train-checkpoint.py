...
class StreamToLogger:
    ...
    def encoding(self):
        if self._fallback_stream is not None:
            return getattr(self._fallback_stream, "encoding", "utf-8")
        return "utf-8"

    def isatty(self):
        if self._fallback_stream is not None and hasattr(self._fallback_stream, "isatty"):
            return self._fallback_stream.isatty()
        return False

    def fileno(self):
        if self._fallback_stream is not None and hasattr(self._fallback_stream, "fileno"):
            return self._fallback_stream.fileno()
        raise OSError("No file descriptor for StreamToLogger")
...
def setup_file_logging(log_file: Path, redirect_std: bool = True, also_console: bool = True):
    """Configure logging to file (and optionally console), with optional stdout/stderr redirection."""
    ...
    if redirect_std:
        sys.stdout = StreamToLogger(logger, logging.INFO, fallback_stream=original_stdout)
        sys.stderr = StreamToLogger(logger, logging.INFO, fallback_stream=original_stderr)

    print("=" * 80)
    print(f"Run started: {datetime.now().isoformat(timespec='seconds')}")
    print(f"Logging to: {log_file}")
    print(f"tmux detected: {'yes' if os.environ.get('TMUX') else 'no'}")
    print("=" * 80)
...
def main():
    ...
    parser.add_argument(
        "--no-stdout-redirect",
        action="store_true",
        help="Do not redirect stdout/stderr into logger; useful when tailing output directly.",
    )
    parser.add_argument(
        "--no-console-log",
        action="store_true",
        help="Log only to file (disable console logging).",
    )
    args = parser.parse_args()

    setup_file_logging(
        Path(args.log_file),
        redirect_std=not args.no_stdout_redirect,
